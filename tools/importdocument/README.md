# Q-Pilot Chat Import Document App
...
## Running the app against a local Q-Pilot instance
...
## Running the app against a deployed Q-Pilot instance
...
1. Create a registered app in Azure Portal (https://learn.microsoft.com/azure/active-directory/develop/quickstart-register-app).
   ...
   > Note that this needs to be a separate app registration from those you created when deploying Q-Pilot.
   ...
   - Select **`Accounts in this organizational directory only (Microsoft only - Single tenant)`** as the supported account
     type for this sample.
      ...
      > **IMPORTANT:** The supported account type should match that of the backend's app registration. If you changed this setting to allow allow multitenant and personal Microsoft accounts access to your Q-Pilot application, you should change it here as well.
...
2. **Run** the following command to import a document to the app under the global document collection where
   ...
   > Note that both of these commands will open a browser window for you to sign in to ensure you have access to the Q-Pilot service.
